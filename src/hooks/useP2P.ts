...
export function useP2P(options: UseP2POptions = {}): UseP2PReturn {
  ...
  const initialize = useCallback(() => {
    ...
    const manager = getP2PManager(walletAddress, config);
    ...
    socketManager.setOnSignalOffer((data) => {
      ...
      manager.handleSignal(data.fromWallet, signalData);
    });
  ...
  }, [publicKey, config, handleMessage, handleStateChange, handleSignal, handleError]);
  ...
  const sendMessage = useCallback(
    (peerId: string, encrypted: string, nonce: string, channelId?: string): boolean => {
      const manager = p2pManagerRef.current;
      if (!manager || !publicKey) return false;
  ...
  // Initialize on mount when wallet is connected
  useEffect(() => {
  ...
  }, [autoConnect, walletConnected, publicKey, initialize, destroy]);
