...
import type { Channel as ChannelType, Community, CurrentUser } from '@/components/layout/Sidebar';
...
export default function Channel() {
  ...
  const fetchChannelData = useCallback(async () => {
    ...
    try {
      ...
      if (!channelsRes.ok) {
        if (channelsRes.status === 404) navigate('/app');
        return;
      }
  ...
  const handleSelectChannel = useCallback(
    (newChannelId: string) => navigate(`/app/community/${communityId}/channel/${newChannelId}`),
    [communityId, navigate],
  );
  ...
  const handleMemberClick = useCallback(
    (member: Member) => {
      if (member.publicId !== publicId) navigate(`/app/dm/${member.publicId}`);
    },
    [publicId, navigate],
  );
  ...
  const sidebarChannels: ChannelType[] = useMemo(
    () =>
      channels.map((c) => ({
        id: c.id,
        name: c.name,
