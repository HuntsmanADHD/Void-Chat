...
/** Truncate a wallet/public ID for display */
function truncateWallet(id: string, chars = 4): string {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-8 py-16 text-center">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button className="flex items-center justify-center gap-2 px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-medium rounded-xl transition-colors border border-zinc-700/50"><Users className="w-5 h-5" />Explore Communities</button>
      </div>
    </div>
  );
});
        ...
        {dms.slice(0, 5).map((dm) => (
          <button key={dm.id} onClick={() => onSelectDM(dm.recipientId)} className="w-full flex items-center gap-3 p-3 bg-zinc-900/50 hover:bg-zinc-800/70 rounded-lg transition-colors border border-zinc-800/30">
            ...
            <div className="flex-1 text-left">
              <div className="font-medium text-zinc-100" title={dm.recipientId}>{truncateWallet(dm.recipientId)}</div>
...
export default function AppDashboard() {
  ...
  const handleSelectCommunity = useCallback((communityId: string) => { navigate(`/app/community/${communityId}`); }, [navigate]);
  ...
  const handleCreateCommunity = useCallback(async (data: CreateCommunityFormData) => {
    ...
    try {
      ...
      if (response.success) { setShowCreateModal(false); showSuccess('Community created successfully!'); navigate(`/app/community/${response.data?.id}`); } else { setCreateError(response.error?.message || 'Failed to create community'); }
