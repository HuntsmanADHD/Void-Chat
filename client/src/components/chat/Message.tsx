...
/**
 * Parse a file message from decrypted content.
 * Returns the parsed file metadata or null if not a file message.
 */
function parseFileMessage(content: string): { type: 'file'; fileName: string; fileSize: number; fileType: string; encryptedData: string; nonce: string } | null {
  try {
    const parsed = JSON.parse(content);
    if (parsed && parsed.type === 'file' && typeof parsed.fileName === 'string') {
      return parsed;
    }
  } catch {
...
/**
 * File attachment display for file messages
 */
function FileAttachment({ fileName, fileSize, fileType }: { fileName: string; fileSize: number; fileType: string }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50 max-w-sm">
      <div className="w-10 h-10 rounded-lg bg-zinc-700/50 flex items-center justify-center text-zinc-400 flex-shrink-0">
        {isImage ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-zinc-500">{sizeStr} &middot; {fileType.split('/')[1] || fileType}</div>
      </div>
    </div>
  );
}
...
export const Message = React.memo(function Message({
  ...
  const displayName = useMemo(() => {
    return truncateWallet(sender.publicId);
  }, [sender.publicId]);
  ...
  return (
    <div className={`group relative flex items-start gap-3 px-4 py-1 hover:bg-zinc-900/50 transition-colors ${className}`}>
      {showSender ? (
        <div className="cursor-pointer" onClick={() => navigate(`/app/wall/${sender.publicId}`)}>
          <Avatar
      ...
      <div className="flex-1 min-w-0">
        ...
        {showSender && (
          <div className="flex items-center gap-2 mb-0.5">
            <span
              ...
              onClick={() => navigate(`/app/wall/${sender.publicId}`)}
            >
              {displayName}
            </span>
        ...
        <div className={`text-zinc-300 break-words ${!showSender ? 'ml-0' : ''}`}>
          {isDecryptionError ? (
            ...
            (() => {
              const fileMsg = parseFileMessage(decryptedContent);
              if (fileMsg) {
                return <FileAttachment fileName={fileMsg.fileName} fileSize={fileMsg.fileSize} fileType={fileMsg.fileType} />;
              }
