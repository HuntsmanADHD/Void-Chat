 ...
 *
 * TODO [SECURITY H-9]: Migrate key storage from localStorage to IndexedDB.
 * localStorage is synchronous, has a ~5 MB limit, and is accessible to any JS
 * running on the same origin (including XSS payloads). IndexedDB supports
 * structured cloning of binary data and, combined with the Web Crypto API's
 * non-extractable CryptoKey objects, would prevent key material from being
 * read by injected scripts. Channel keys stored here are already encrypted
 * with the user's keypair, but the keypair material itself and public key
 * cache would benefit from the stronger isolation IndexedDB + CryptoKey
 * provides. This migration is tracked as a separate work item due to scope.
 */
...
export async function fetchPublicKey(
  ...
  try {
    const response = await apiClient.get<UserKeyInfo>(`/api/users/${publicId}/public-key`);
...
export async function getPublicKeys(
  ...
  if (toFetch.length > 0) {
    try {
      const response = await apiClient.post<UserKeyInfo[]>('/api/users/public-keys', { publicIds: toFetch });
