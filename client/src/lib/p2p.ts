...
// Prevents senderId spoofing on the data channel (M-12 fix).
// Uses Ed25519 (nacl.sign.detached) over a canonical representation of the
// message fields. The signature field itself is excluded from signing.
// =============================================================================

/**
 * Build the canonical byte string for signing a P2P message.
 * Includes all semantically meaningful fields in a deterministic order.
 * The `signature` field is excluded (it's what we're computing/verifying).
 * The `fileOffer` and `fileChunk` fields are included via JSON serialization
 * to ensure they cannot be tampered with.
 */
function getSignablePayload(message: P2PMessage): Uint8Array {
  const parts: string[] = [
    message.id,
    message.type,
    message.senderId,
    ...
    message.nonce ?? '',
    message.channelId ?? '',
    message.fileOffer ? JSON.stringify(message.fileOffer) : '',
    message.fileChunk ? JSON.stringify(message.fileChunk) : '',
  ];
  return new TextEncoder().encode(parts.join('\0'));
}

/**
 * Sign a P2P message using an Ed25519 signing secret key.
 * Mutates the message in-place by setting the `signature` field.
 *
 * @param message - The message to sign (modified in-place)
 * @param signingSecretKey - 64-byte Ed25519 secret key (Uint8Array)
 */
export function signP2PMessage(message: P2PMessage, signingSecretKey: Uint8Array): void {
  const payload = getSignablePayload(message);
  const sig = nacl.sign.detached(payload, signingSecretKey);
  message.signature = naclUtil.encodeBase64(sig);
}

/**
 * Verify the Ed25519 signature on a P2P message.
 *
 * @param message - The message to verify
 * @param signingPublicKey - 32-byte Ed25519 public key (Uint8Array)
 * @returns true if signature is valid, false otherwise
 */
export function verifyP2PMessageSignature(
  message: P2PMessage,
  ...
  try {
    const payload = getSignablePayload(message);
    const sig = naclUtil.decodeBase64(message.signature);
    return nacl.sign.detached.verify(payload, sig, signingPublicKey);
  } catch {
    return false;
  }
}
...
/**
 * Callback to resolve a peer's Ed25519 signing public key from their publicId.
 * Used by the P2P manager to verify incoming message signatures.
 */
export type ResolveSigningKey = (publicId: string) => Uint8Array | null;
...
export class P2PManager {
  ...
  // Ed25519 signing key for outgoing messages (prevents senderId spoofing)
  private signingSecretKey: Uint8Array | null = null;
  // Callback to resolve peer signing public keys for verification
  private resolveSigningKey: ResolveSigningKey | null = null;
  ...
  /**
   * Set the Ed25519 signing secret key for signing outgoing P2P messages.
   * This should be the 64-byte NaCl sign secret key.
   */
  ...
  /**
   * Set the callback for resolving a peer's Ed25519 signing public key.
   * Used to verify incoming message signatures.
   */
  setResolveSigningKey(resolver: ResolveSigningKey): void {
    this.resolveSigningKey = resolver;
  }
  ...
  private setupPeerEvents(peerId: string, peerData: PeerData): void {
    ...
    peer.on('data', (data: Uint8Array) => {
      try {
        ...
        // Handle ping/pong internally (skip signature check for keepalives)
        ...
        // Verify senderId matches the peer we expect on this connection
        // (basic check — the peer connected via signaling as peerId)
        if (message.senderId !== peerId) {
          console.error(`[P2P] senderId mismatch: message claims ${message.senderId}, connection is to ${peerId} — dropping`);
          return;
        }

        // Verify Ed25519 signature if we have a key resolver
        if (this.resolveSigningKey) {
          const signingPubKey = this.resolveSigningKey(message.senderId);
          if (signingPubKey) {
            if (!verifyP2PMessageSignature(message, signingPubKey)) {
              console.error(`[P2P] Signature verification failed for message ${message.id} from ${message.senderId} — dropping`);
              return;
            }
          } else {
            console.warn(`[P2P] No signing key available for ${message.senderId} — cannot verify signature`);
          }
        }
  ...
  sendMessage(peerId: string, message: P2PMessage): boolean {
    ...
    try {
      ...
      if (this.signingSecretKey) {
        signP2PMessage(message, this.signingSecretKey);
      }
